{
  ...
  cout<< ""<< endl;
  cout<< "initializing R3B_BoxCsI_APD_SourceGamma.C macro"<< endl;
  cout<< ""<< endl;
//
...
// 1 = TPhotoElecCompton
  ...
  Int_t nRuns = 100;         // number of the Runs
  cout<< ""<< endl;
  cout<< "nRuns = "<< nRuns <<  endl;
  cout<< ""<< endl;
//Histos and stuff
  Double_t gammaE = 0.662;  //energy of the incomming gamma (MeV) CAMBIAR para E=1MeV
  Double_t nPhotMeV = 60000; // intrinsic light yield 
//  Int_t nBins = (int)(gammaE);
...
// verified the  absorption length parameter La????, see TOpticalMaterial 
// if you ou intend to call IsIsotropic(), give only the La and Lb,Lc=-1
//                                                      
...
//DEdx_GammaInCsITl by Hector - USC, using Geant 4, gives the value
...
//dependency provided. CMS setup
  glue->IsIsotropic(1.43); //USC documentation value
//
...
// These values extrapolated from indications given by Hammamatsu.
  ...
  epoxy = new TOpticMaterial("Epoxy","Epoxy window for APD",kFALSE,1.0,0.5);
  epoxy->IsIsotropic(1.54);//USC documentation value
//
...
//absorption.  The revetment is 98.0% diffusing.
  const Double_t PropDiffus = 1.0; //all photons are reflected.
                                   //proportion [0..1] of photons which are diffused instead
                                   //of reflected. All non-absorbed light which is not diffused 
                                   // is assumed to be reflected.
  TRevetment *vm2000;
  vm2000 = new TRevetment("vm2000","vm2000","Air",PropDiffus,
			  "RIndexRev_vm2000", "IIndexRev_vm2000",1.0,0.0);//8th arg, There is no suppl. absorp.
...
//part of the revetment a supplementary absorption of 5%!
  ...
  vm2000 = new TRevetment("badvm2000","badvm2000","Air",PropDiffus,
			  "RIndexRev_vm2000", "IIndexRev_vm2000",1.0,0.0); //0.05 - There is no supplementary
//
...
//imaginary part of index of refraction varying in function of wavelength. Now, we consider the fixe value 1.3.
//The magnetic permeability is 1, independent of wavelength. There is no diffusion 
//and supplementary absorption. Teflon is 99% diffuse refletance
  TRevetment *teflon;
  teflon = new TRevetment("teflon","teflon","Air",1.0,1.3, 1.3,1.0,0.0);//There is no supplementary
//
//  Definition of the wrapping "Revcaps", describing the surface of the capsule containing
//the APD, which is in contact with face 5 of the crystal.supplementary absorption of 0.1
  TRevetment *revcaps;
  revcaps  = new TRevetment("Revcaps","Revcaps","Glue",1.0,"RIndexRev_CMScapsula",
			    "IIndexRev_CMScapsula",1.0,0.1);//there is supplementary absorption
//
...
// We define the Glue and Epoxy position
  const Double_t glue_dz    = 0.0015; //half width of glue (0.3mm) between crystal and APD [Montecchi] in Oz
  const Double_t epoxy_dz   = 0.0075; //half width of epoxy window (0.15mm - Bernard) in Oz.
...
// Definition of the position for APD
  ...
  const Double_t apd_dz  = 0.0225;    //half-width of silicium of APD - 450 microns 
//const Double_t apd_dz  = 0.0050;    //half-width of silicium of APD - 100 microns 
  cout<< ""<< endl;
  cout<< " Definition of APD, Si apd_dz = " << apd_dz << endl;
  cout<< ""<< endl;
...
//
//Setting depolish and bevellings, and frame of APDs as revetment for faces 4/5
  const Double_t ThetaDepls   = 31.48; //31.48 - value between (0,90)
...
//  
  cout<< ""<< endl;
  cout<< "initializing Generate Events"<< endl;
  cout<< ""<< endl;
...
//
  T3Vector StartCasc(0.0, 0.0, -crystal_dz - src_dz - src_dist); //source position(x,y,z)
...
// Results
//
  TH1F* spectrum = new TH1F("spectrum", "Photons seen", nBins, 0, nBins*1.0);
//
// Runs
//
  ...
  //if (gCs->fNpSeen) spectrum->Fill((gCs->fNpSeen));
   if (gCs->fNpSeen) spectrum->Fill(1*(gCs->fNpSeen));
